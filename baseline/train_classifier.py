...
def train_SVC(X_train, y_train, X_val, y_val, do_cv=False):
    ...
    best_param = {}
    best_param['gamma'] = 0.00001
    best_param['kernel'] = 'rbf'
    best_param['C'] = 10.0

    #SVM model
    clf = SVC(C=best_param['C'], kernel='rbf', gamma=best_param['gamma'], probability=True, 
              cache_size=1000, random_state=777)
    ...
    if do_cv:
        ...
        for z in ParameterGrid(grid):
            ...
            clf.set_params(**z)
            clf.fit(X_scaled[idx, :], y_train[idx])
    ...
    print('gamma:{}, kernel:{}, nu:{} **'.format(best_param['gamma'],
                                                                     best_param['kernel'],
                                                                     best_param['C']))
    svm_model = {}
    ...
    idx = np.random.choice(range(len(X_scaled)), size=5000, replace=False)
    X_scaled = X_scaled[idx, :].copy()
    y_train = y_train[idx]
    idx = np.random.choice(range(len(X_test_scaled)), size=np.min([5000, len(X_test_scaled)]), replace=False)
...
if __name__ == '__main__':
    ...
    svm_model = train_forgery_detector('/data/home/shruti/voxceleb/fabnet/leaders',
                               None, 
