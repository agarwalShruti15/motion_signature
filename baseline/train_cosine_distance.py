...
feat_fldrs = ['fabnet_metric']
reslt_prefixs = ['results_cosine/fabnet_metric']
...
def get_dataset_equal(vid_len, train):
    ...
    for k in list(train_lbl1.keys()):
        ridx = np.random.choice(np.arange(len(train_lbl1[k])), np.min([max_tr_count, len(train_lbl1[k])]), replace=False)
        if X_train is None:
    ...
    for k in list(test_lbl1.keys()):
        ridx = np.random.choice(np.arange(len(test_lbl1[k])), np.min([max_tr_count, len(test_lbl1[k])]), replace=False)
        if X_test is None:
...
if __name__ == '__main__':
    ...
    for vid_len in vid_lens:
        ...
        for f in range(len(feat_fldrs)):
            ...
            reslt_prefix = reslt_prefixs[f] + '_' + str(vid_len) + '/'
            ...
            for l_name in train_all.keys():
                ...
                idx = np.random.choice(range(len(X_train[y_train==1, :])), 
                                       size=np.min([50000, len(X_train[y_train==1, :])]), replace=False)      
                
                fig_fldr = '{}_{}'.format(reslt_prefix, train_name)
                os.makedirs(fig_fldr, exist_ok=True)
                print(X_train.shape)
        
                # no training, simply compute the minimum cosine distance from the training data
                pred_prob = np.max(cosine_similarity(np.array(X_train)[y_train==1, :][idx, :], np.array(X_test), dense_output=True), axis=0)                    
