...
rs = 777
np.random.seed(rs)
vid_lens = [100]#int(10*30) #30 fps
obj_bsfldr = 'obj' #'/data/home/ec2-user/projects/deep_fakes/obj'
feat_fldrs = ['ldrs_ms_resnet50_synFake']
reslt_prefixs = ['results/ldrs_ms_resnet50_synFake']
gammas = [0.1]
doParallel = False
do_cv = False
...
def get_dataset_equal(vid_len, train):
    ...
    for i in np.argwhere(train['lbl']==1).ravel():
        ...
        db = u.load_obj('{}/{}'.format(obj_bsfldr, feat_fldr), '{}_{}'.format(vid_len, train['db'][i]))
        try:
...
def get_all_train_cases_withfaceswap():

    train_all = {}
    train_all['bernie'] =  {} #leader name
    train_all['bernie']['db'] = np.array(['bs', 'bs_faceswap']) #, 'bs_imposter', 'bs_faceswap'
    train_all['bernie']['lbl'] = np.array([1, 0])
    train_all['bernie']['perc'] = np.array([0.9, 0.8])
    train_all['bernie_imp'] = {}
    train_all['bernie_imp']['db'] = np.array(['bs', 'bs_faceswap', 'bs_imposter']) #, 'bs_imposter', 'bs_faceswap'
    ...
    train_all['bernie_fc']['perc'] = np.array([0.9, 0, 1])

    train_all['ew'] = {} #leader name
    train_all['ew']['db'] = np.array(['ew', 'ew_faceswap']) #
    train_all['ew']['lbl'] = np.array([1, 0])
    train_all['ew']['perc'] = np.array([0.9, 0.8])
    train_all['ew_imp'] = {}
    train_all['ew_imp']['db'] = np.array(['ew', 'ew_faceswap', 'ew_imposter']) #
    ...
    train_all['ew_fc']['perc'] = np.array([0.9, 0, 1])

    train_all['hc'] = {}#leader name
    train_all['hc']['db'] = np.array(['hc', 'hc_faceswap'])
    train_all['hc']['lbl'] = np.array([1, 0])
    train_all['hc']['perc'] = np.array([0.9, 0.8])
    train_all['hc_fc'] = {}
    train_all['hc_fc']['db'] = np.array(['hc', 'diff_test', 'hc_faceswap'])
    train_all['hc_fc']['lbl'] = np.array([1, 0, 0])
    train_all['hc_fc']['perc'] = np.array([0.9, 0, 1])
    train_all['hc_imp'] = {}
    train_all['hc_imp']['db'] = np.array(['hc', 'hc_faceswap', 'hc_imposter'])
    train_all['hc_imp']['lbl'] = np.array([1, 0, 0])
    train_all['hc_imp']['perc'] = np.array([0.9, 1, 0])

    train_all['trump'] =  {} #leader name
    train_all['trump']['db'] = np.array(['dt_week', 'dt_rndm', 'dt_faceswap'])
    train_all['trump']['lbl'] = np.array([1, 1, 0])
    train_all['trump']['perc'] = np.array([0.9, 0.9, 0.8])
    train_all['trump_imp'] = {}
    train_all['trump_imp']['db'] = np.array(['dt_week', 'dt_rndm', 'dt_faceswap', 'dt_imposter']) 
    ...
    train_all['trump_fc']['perc'] = np.array([0.9, 0.9, 0, 1])
    train_all['trump_lipsyn'] = {}
    train_all['trump_lipsyn']['db'] = np.array(['dt_week', 'dt_rndm', 'dt_faceswap', 'trump_fake']) 
    ...
    train_all['obama']['db'] = np.array(['bo', 'bo_faceswap'])#, 'bo_UWfake', 'bo_imposter']) #
    train_all['obama']['lbl'] = np.array([1, 0])
    train_all['obama']['perc'] = np.array([0.9, 0.8])
    train_all['obama_UWFake'] = {}
    train_all['obama_UWFake']['db'] = np.array(['bo', 'bo_faceswap', 'bo_UWfake'])#, 'bo_UWfake', 'bo_imposter']) #
    train_all['obama_UWFake']['lbl'] = np.array([1, 0, 0])
    train_all['obama_UWFake']['perc'] = np.array([0.9, 1, 0])
    train_all['obama_imp'] = {}
    train_all['obama_imp']['db'] = np.array(['bo', 'bo_faceswap', 'bo_imposter'])#, 'bo_UWfake', 'bo_imposter']) #
    ...
    train_all['obama_fc']['perc'] = np.array([0.9, 0, 1])
    ...
    for vid_len in vid_lens:
        ...
        for gamma in gammas:
            ...
            for f in range(len(feat_fldrs)):
                ...
                feat_fldr = feat_fldrs[f]
                reslt_prefix = reslt_prefixs[f] + '_' + str(gamma) + '_' + str(vid_len) + '/'
        
                print('obj_fldr : {}  \ndoParallel : {} \nfeat_fl :{} \nvid_len : \n{} reslt_prefix : {} '.format(obj_bsfldr,
                                                                                                                  doParallel,
                                                                                                                  feat_fldr,
                                                                                                                  vid_len,
                                                                                                                  reslt_prefix))
                train_all = get_all_train_cases() #get_all_train_cases_withfaceswap() #
                for l_name in train_all.keys():
            
                    train = train_all[l_name]
                    print(l_name)
                    #get the dataset
                    X_train, y_train, X_test, y_test, fv_X_test, fv_y_test, train_name, test_name = get_dataset_equal(vid_len, train)
                    ...
                    if linear_model is None:
                        print('training...')
                        linear_model = u.train_ovr(np.array(X_train)[y_train==1, :], np.array(X_train), y_train, gamma=gamma, nu=0.1, do_cv=do_cv)
                        #train auc
