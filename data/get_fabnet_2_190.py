...
def lndmrk_to_df(infile):
    ...
    valid_idx = cur_df[' confidence']>0.0
    clean_df = cur_df.loc[valid_idx, :]
    return clean_df, valid_idx
...
def tmp_get_full_df(infile):
    ...
    lndmrk_df, valid_idx = lndmrk_to_df(csv_files)

    # combine the transcript and landmark files
    out_df = combine_lip_opening(lndmrk_df)

    return out_df, valid_idx


def one_vid_feat(in_fldr, fl_n, fabnet_file, out_file):

    cur_csv = os.path.join(in_fldr, fl_n )
    assert not os.path.exists(out_file)
    try:
        
        fabnet_feat = np.load(fabnet_file)
        if len(fabnet_feat.shape)>2:
            fabnet_feat = fabnet_feat[:, :, 0].copy()
            
        full_df, valid_idx = tmp_get_full_df(cur_csv)
        
        if full_df is None:
            return
        out_emb = np.hstack((np.array(full_df[feat_nm]), fabnet_feat[valid_idx, :]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fabnet_path', type=str, help='path to fabnet embedding')
    ...
    for p in paths:
        ...
        if os.path.exists(os.path.join(fabnet_path, '_'.join(dirname.split('/')) + '_' + fl_n + '.npy')):
            fabnet_file = os.path.join(fabnet_path, '_'.join(dirname.split('/')) + '_' + fl_n + '.npy')
        else:
            fabnet_file = os.path.join(fabnet_path, dirname, fl_n + '.npy')
            
        if not os.path.exists(out_file):
            full_struct.append((os.path.join(bs_fldr, dirname), fl_n, fabnet_file, out_file))
