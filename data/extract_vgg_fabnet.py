...
def get_out_file_nm(bs_fldr, ofd, dirname, fl_n):
    ...
    return os.path.join(out_fldr, fl_n + '.npy')
...
if __name__ == '__main__':
    ...
    for dirname, dirnames, filenames in os.walk(bs_fldr):
        ...
        for vi in range(len(vid_files)):
            ...
            fab_outfile = get_out_file_nm(bs_fldr, o_fab, dirname, fl_n)
            vgg_outfile = get_out_file_nm(bs_fldr, o_vgg, dirname, fl_n)

            if not os.path.exists(fab_outfile) or not os.path.exists(vgg_outfile):
                full_struct.append((dirname, fl_n, vgg_model, fab_model, openface_path, vgg_outfile, fab_outfile))
