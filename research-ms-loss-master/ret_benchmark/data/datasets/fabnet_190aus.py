...
@DATA_LOADER.register('FabNet_190aus')
class FabNet2_190ausDataLoader(Dataset):
    """
    ...
    def __getitem__(self, index):
        ...
        # get the first 20 features to the y label and rest to fabnet embeddings
        y_lbl = img[:, :20]
        fabnet = img[:, 20:]
        ...
        while len(out_X)<1:
            ...
            r_idx = np.random.choice(np.arange(len(fabnet)-self.T+1), 1)[0]
            ...
            if np.sum(np.isnan(corr))>0 and i < len(fabnet):
                i = i+1
                continue
            else:
                out_X = fabnet[r_idx:r_idx+self.T, :].copy()
                out_Y = corr.ravel().copy()
                out_Y[np.isnan(corr)] = 0
        
        # unit sphere for fabnet
        out_X = out_X/ np.linalg.norm(out_X, axis=1, keepdims=True)
