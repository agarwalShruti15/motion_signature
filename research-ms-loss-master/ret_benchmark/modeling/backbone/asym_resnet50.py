...
def conv3x1(in_planes, out_planes, stride=1, groups=1, dilation=1):
    ...
    return nn.Conv2d(in_planes, out_planes, kernel_size=(3, 1), stride=(stride,1),
                     padding=(dilation,0), groups=groups, bias=False, dilation=(dilation,1))
...
@registry.BACKBONES.register('asym_resnet50')
class AsymResNet50(nn.Module):

    def __init__(self):
        super(AsymResNet50, self).__init__()
        ...
        self.conv1 = nn.Conv2d(1, self.inplanes, kernel_size=(7, 1), stride=(2, 1), padding=(3, 0),
                               bias=False)
